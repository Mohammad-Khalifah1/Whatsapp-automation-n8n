...
function buildMessageProcessor() {
  ...
  nodes.push(
    codeNode(
      ...
      'apply-echo',
      [560, 560],
      ['conversation.js', 'labels.js'],
      [
        "const echo = $('Route By Event Kind').item.json;",
        '// Labels become codes the moment a row is read, so nothing below ever',
        '// compares an Arabic string (see scripts/lib/labels.js).',
        'const rows = $input.all().map((i) => normalizeConversationRow(i.json))',
        ...
        '  // Codes become labels on the way back to the sheet.',
        '  conversation_update: conversationRowToSheet(',
        '    Object.assign({ conversation_id: existing.conversation_id }, update), lang),',
        '}) }];',
      ].join('\n')
    )
  );
    ...
    parameters: {
      ...
      columns: {
        ...
        value: {
          ...
          // From the labelled update, not a literal: on an Arabic sheet a
          // hard-coded 'outbound' is the one English word in the column.
          last_message_direction: '={{ $json.conversation_update.last_message_direction }}',
...
function buildConversationAndAssignment() {
  ...
  nodes.push(
    codeNode(
      ...
      'decide-conv',
      [-40, 0],
      ['conversation.js', 'phone.js', 'labels.js'],
      [
        "const event = $('When Called By Processor').first().json;",
        '// Labels become codes the moment a row is read (scripts/lib/labels.js).',
        'const rows = $input.all().map((i) => normalizeConversationRow(i.json))',
        '  .filter((j) => j && j.conversation_id);',
        '',
  ...
  nodes.push(
    codeNode(
      ...
      'select-agent',
      [740, -120],
      ['assignment.js', 'conversation.js', 'labels.js'],
      [
        ...
        '  .map((i) => normalizeConversationRow(i.json))',
        '  .filter((r) => r && r.conversation_id);',
  ...
  nodes.push(
    codeNode(
      ...
      [
        ...
        'const nowIso = localIso();',
        "// What language this sheet is kept in. Default 'en', where a label IS",
        '// the code, so an English sheet is written exactly as it always was.',
        "const lang = String($env.SHEET_LANGUAGE || 'en').trim().toLowerCase();",
        '',
        ...
        '  // Codes become labels on the way back to the sheet.',
        '  const sheetRow = conversationRowToSheet(row, lang);',
        "  return [{ json: Object.assign({}, ctx, { write_row: sheetRow, sheet_operation: 'append' }) }];",
        '}',
        '',
        ...
        '',
        'const sheetRow = conversationRowToSheet(merged, lang);',
        "return [{ json: Object.assign({}, ctx, { write_row: sheetRow, sheet_operation: 'update' }) }];",
      ].join('\n')
    )
  );
...
function buildOutgoingMessage() {
  ...
  nodes.push(
    codeNode(
      ...
      'interpret-send',
      [480, -100],
      ['security.js', 'labels.js'],
      [
        "const request = $('Request Valid?').first().json;",
        'const response = $input.first().json;',
        'const nowIso = localIso();',
        "// Conversations is the tab people read, so its cells carry labels.",
        "// Messages and this workflow's own answer keep the codes.",
        "const lang = String($env.SHEET_LANGUAGE || 'en').trim().toLowerCase();",
        '',
        ...
        "  conversation_status: toLabel('status', 'REPLIED', lang),",
        "  conversation_direction: toLabel('direction', 'outbound', lang),",
        "  conversation_via: toLabel('via', 'API', lang),",
        '} }];',
      ].join('\n')
    )
  );
  ...
  nodes.push({
    parameters: {
      ...
      columns: {
        ...
        value: {
          conversation_id: '={{ $json.conversation_id }}',
          status: '={{ $json.ok ? $json.conversation_status : undefined }}',
          last_message: '={{ $json.ok ? $json.text : undefined }}',
          last_message_id: '={{ $json.ok ? $json.message_id : undefined }}',
          last_message_direction: '={{ $json.ok ? $json.conversation_direction : undefined }}',
          ...
          last_reply_via: '={{ $json.ok ? $json.conversation_via : undefined }}',
...
function buildUnassignedRetry() {
  ...
  nodes.push({
    parameters: {
      ...
      documentId: { __rl: true, value: '={{ $env.GOOGLE_SHEET_ID }}', mode: 'id' },
      sheetName: { __rl: true, value: 'Conversations', mode: 'name' },
      // No status filter here. The node compares the cell's TEXT, so on an
      // Arabic sheet 'WAITING_FOR_AGENT' matches nothing and the queue is
      // never retried. Assign Waiting Queue turns each row back into codes
      // and filters on the code instead - and reads no extra rows doing it,
      // because this node fetches the tab and filters in memory either way.
      options: { returnAllMatches: true },
    },
    id: 'read-waiting',
  ...
  nodes.push(
    codeNode(
      ...
      [
        ...
        '  // Labels become codes on read (scripts/lib/labels.js).',
        '  .map((i) => normalizeConversationRow(i.json))',
        ...
        "    status: toLabel('status', 'UNANSWERED', lang),",
        "    unassigned_reason: '',",
        '    updated_at: nowIso,',
...
function buildReplyFromSheet() {
  ...
  nodes.push(
    codeNode(
      ...
      'find-pending',
      [-140, 0],
      ['phone.js', 'window.js', 'reply-guard.js', 'templates.js', 'labels.js'],
      [
        ...
        '// Labels become codes the moment a row is read; the outcome write',
        '// turns them back (scripts/lib/labels.js).',
        'const rows = $input.all().map((i) => normalizeConversationRow(i.json))',
        ...
        '      skip: true,',
        "      reply_status: toLabel('reply_status', blocked.status, lang),",
  ...
  nodes.push(
    codeNode(
      ...
      [
        ...
        "  // Back to the sheet's own words: these are the values a failed",
        '  // send rewrites unchanged, and they must not land as codes on an',
        '  // Arabic sheet just because the read turned them into codes.',
        '  const row = conversationRowToSheet(request.source_row || {}, lang);',
        '',
        ...
        '    ok,',
        '    message_id: messageId,',
        "    reply_status: toLabel('reply_status', ok ? 'SENT' : (windowClosed ? 'WINDOW_CLOSED' : 'FAILED'), lang),",
        "    reply_error: apiError ? ('[' + apiError.code + '] ' + String(apiError.message).slice(0, 200)) : '',",
        '    sent_at: nowIso,',
        "    new_status: ok ? toLabel('status', 'REPLIED', lang) : (row.status || ''),",
        ...
        "    new_last_message_direction: ok ? toLabel('direction', 'outbound', lang) : (row.last_message_direction || ''),",
        "    new_last_agent_message_at: ok ? nowIso : (row.last_agent_message_at || ''),",
        "    new_unread: ok ? 'FALSE' : (row.unread || ''),",
        "    new_last_reply_via: ok ? toLabel('via', request.is_template ? 'TEMPLATE' : 'SHEET', lang) : (row.last_reply_via || ''),",
...
function buildArchive() {
  ...
  nodes.push(
    codeNode(
      ...
      ['conversation.js', 'labels.js'],
      [
        '// Labels become codes the moment a row is read; the copy into the',
        '// Archive at the end turns them back (scripts/lib/labels.js).',
        "const lang = String($env.SHEET_LANGUAGE || 'en').trim().toLowerCase();",
        'const rows = $input.all().map((i) => normalizeConversationRow(i.json))',
        '  .filter((r) => r && r.conversation_id);',
        '',
        ...
        '}));',
        '',
        '// The Archive is a tab people read, so a row arrives in it written the',
        '// same way it was written in Conversations.',
        "return batch.map((r) => ({ json: Object.assign({}, conversationRowToSheet(r, lang), {",
        '  archived_at: localIso(now),',
        '}) }));',
      ].join('\n')
    )
  );
