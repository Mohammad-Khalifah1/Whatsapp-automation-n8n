...
| C-38 | The live scripts compared English codes | `verify-live.js`, `verify-archive.js`, `scenario-multi-agent.js`, `show-sheet.js` | Every check fails on a sheet kept in another language - a false alarm about a system that is working - and a status typed by a test lands as a value the column rejects | They compare `toCode(field, cell)` and write `toLabel(field, code, lang)`; a source rule keeps it that way, since running them needs the deployment. **Fixed with V2-12** | V2-12 |
