...
| Two customers, one n8n instance | Yes — appends use `INSERT_ROWS` ([ARCHITECTURE.md](ARCHITECTURE.md#messages-arriving-at-the-same-instant)); a duplicate conversation is folded by workflow 8. Without the service account in `.env`, appends fall back to the Sheets node and can collide |
